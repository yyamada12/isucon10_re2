...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	query = `SELECT id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features, popularity FROM estate WHERE (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) ORDER BY minuspopularity ASC, id ASC LIMIT ?`
	err = db.Select(&estates, query, a, b, b, a, Limit)
	if err != nil {
		if err == sql.ErrNoRows {
			return c.JSON(http.StatusOK, EstateListResponse{[]Estate{}})
		}
